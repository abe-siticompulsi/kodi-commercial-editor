...
| 7 | **Seed strategy = verified-only.** Invariant: *nothing auto-skips that a human hasn't confirmed.* | Automatic detection is imprecise by definition; a silent wrong jump mid-movie is worse than an unpressed button. As playback nears a draft marker, a popup offers "commercial ahead — skip & confirm?" — one click skips now AND writes the EDL. Unconfirmed drafts remain whatever the user's Plex client already surfaces (e.g. a skip button). |
