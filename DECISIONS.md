...
| 3 | Kodi-file ↔ Plex-ratingKey mapping | **Resolved (design; from PKC source research).** Primary: the playing item's Kodi library DbId → PKC's `plex.db` (`movie`/`episode` tables, `kodi_id` column; `plex_id` IS the ratingKey). Fallback: file path → Kodi's `MyVideosNNN.db` (`files` JOIN `path`, dir+filename split) → `kodi_fileid` → `plex.db`. Server+token are borrowed from PKC's settings (`ipaddress`/`port`/`https`/`accessToken` — the per-server token PKC itself uses) → **zero-config for PKC users**. PKC keeps its DBs in WAL mode: short-lived read-only connections (`file:…?mode=ro`) are safe. Non-PKC setups need manual server/token settings — deferred until someone needs it. |
